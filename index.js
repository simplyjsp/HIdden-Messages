...
function encodeMessage() {
    const message = document.getElementById('messageInput').value + String.fromCharCode(0); // Append null character to signify end of message
    ...
    for (let i = 0; i < messageBinary.length; i++) {
        ...
        dataIndex += 4; // Move to the next pixel's first channel (RGBA)
    }

    ctx.putImageData(imageData, 0, 0);
    showDownloadButton(); // Function to show the download button
}

function showDownloadButton() {
    const downloadBtn = document.getElementById('downloadBtn');
    downloadBtn.style.display = 'inline'; // Ensure the button is visible
...
function decodeMessage() {
    ...
    for (let i = 0; i < data.length; i += 4) {
        ...
        if (binaryMessage.length === 8) {
            ...
            if (charCode === 0) { // Null character signifies the end of the message
                break;
            }
